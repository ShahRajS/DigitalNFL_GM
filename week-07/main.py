...
# Load and programmatically extract the PDF Draft Packet context
pdf_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), '2025-San-Francisco-49ers-Draft-Packet.pdf')
try:
    with open(pdf_path, 'rb') as f:
        reader = PyPDF2.PdfReader(f)
        draft_packet_context = ""
        for page in reader.pages:
            draft_packet_context += page.extract_text() or ""
        # Clean curly braces to prevent ADK parser KeyError crashes
        draft_packet_context = draft_packet_context.replace("{", "[").replace("}", "]")
except Exception as e:
    draft_packet_context = f"Error loading draft packet PDF: {e}"
...
# 2. Define the Agent with Memory Tools & Injected Draft Packet Context
draft_scout = Agent(
    name="Draft_Scout",
    model=model,
    description="A scout that manages the persistent draft room board, queries the database, and has access to the 2025 San Francisco 49ers Draft Packet.",
    tools=[add_player_to_draft_board, remove_player_from_draft_board, view_draft_board, execute_db_query, execute_web_search],
    instruction=f"""You are 'YourGM,' an elite NFL Scout managing the team's Draft Board.
Your goal is to maintain the Draft Room's memory, use the database when requested, and provide scouting intelligence and draft facts using the team's draft packet.

You have direct access to the following 2025 San Francisco 49ers Draft Packet context:
=========================================
{draft_packet_context}
=========================================

Guidelines for answering questions using the draft packet:
1. When asked about draft dates, rules/limits, prospect notes, or on-site player stats, you MUST read the draft packet context and answer accurately based on it.
2. If the user asks a question not covered by the draft packet, state that the draft packet does not cover that specific detail.
3. Be professional, highly analytical, and behave like a professional NFL general manager.
